...
from .body_language_math import findDistance, calculate_angle
...
def body_language_analysis_module(InputVideoPath):
    # The pickled sklearn model can be version-sensitive. If it fails to load/predict,
    # return a partial result instead of crashing the whole request.
    try:
        return _body_language_analysis_module_impl(InputVideoPath)
    except Exception as e:
        return {"body_language_error": str(e)}


def _body_language_analysis_module_impl(InputVideoPath):

    directory_path = "./models/pretrained_models"
    model_name = "body_language.pkl"

    file_path = os.path.join(directory_path, model_name)

    # model = load('./models/pretrained_models/body_language.pkl')
