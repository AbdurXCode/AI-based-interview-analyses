...
async def head_driver(video_files: List[UploadFile] = File(...)):
    ...
    for video_file in video_files:
        ...
        # Calculate and update overall analysis
        video_count += 1
        for key in gaze_track_dict:
            overall_analysis["gaze_track"][key] = overall_analysis["gaze_track"].get(key, 0) + gaze_track_dict[key] / video_count
        for key in speaking_analyse_dict:
            if key != "spoken_text":
                overall_analysis["speaking_analysis"][key] = overall_analysis["speaking_analysis"].get(key, 0) + speaking_analyse_dict[key] / video_count
        overall_analysis["speaking_analysis"]["spoken_text"] += speaking_analyse_dict.get("spoken_text", "") + " | "

        print("-------------------- Body Language Analysis start..... ---------------------")
        for key in body_analyse_dict:
            overall_analysis["body_analysis"][key] = overall_analysis["body_analysis"].get(key, 0) + body_analyse_dict[key] / video_count
        ...
        print(f"===================== This {video_file.filename} file run successfully  ====================")

    # Trim the final " | " from the concatenated text
    overall_analysis["speaking_analysis"]["spoken_text"] = overall_analysis["speaking_analysis"]["spoken_text"].rstrip(" | ")
    
    # Add overall analysis to the dictionary
    video_analyse_dictionary["overall analysis of all videos"] = overall_analysis
    return video_analyse_dictionary
